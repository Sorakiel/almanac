...
| `set_logs`           | id, workout_exercise_id, set_number, reps, weight, done, logged_at, session_id — **null session_id = the plan row; a session's rows are what was logged that day**                                                                                                                                     |
| `workout_sessions`   | id, user_id, workout_id, date (local day), started_at, completed_at — **unique(workout_id, date)**; written through `log_workout_set` / `set_workout_session_done`; `workouts.completed_at` mirrors the last finish                                                                                    |
| `quotes`             | id, text, author, text_ru, author_ru — **global, read-only to users**; no `text_ru` hides the quote in Russian                                                                                                                                                                                         |
...
| `user_settings`      | user_id (pk → auth.users), modules jsonb, home_order text[], pinned_tab, theme (`dark`\|`coffee`\|`system`), locale (`en`\|`ru`), sound, updated_at (server-stamped) — **one row per user, created lazily by the client's first upsert; null = no choice; no delete policy (reset = update to nulls)** |
