...
import { groupYearByWeek, type YearDay, type YearWeek } from '@/features/progress/lib/yearActivity'
  ...
  return (
    <section className={cn('rounded-card border bg-surface px-4 py-3.5', className)}>
      <div className="flex items-baseline justify-between">
        <span className="text-footnote font-medium">{t('insights.yearStrip')}</span>
        <span className="num text-caption text-muted-strong">{year}</span>
      </div>

      <div
        className="mt-3 grid h-12 items-end gap-0.5 rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-accent/60 focus-visible:ring-offset-2 focus-visible:ring-offset-surface sm:gap-1"
        style={columns}
        role="group"
        tabIndex={0}
        ...
        {weeks.map((week, i) => {
          ...
          return (
            <span
              ...
              className={cn(
                'relative flex h-full min-w-0 items-end justify-center rounded-sm transition-colors',
                isActive && 'bg-foreground/5',
              )}
            >
              {/* No full-height track: an empty week must not read as a full one. */}
              {week.future ? (
                <span className="h-1 w-full rounded-sm bg-muted-strong/20" />
              ) : week.ratio === null ? (
                <span className="h-1 w-1 rounded-full bg-muted-strong/60" />
              ) : (
                <span
                  className={cn(
                    'w-full origin-bottom rounded-sm motion-safe:animate-cell-in',
      ...
      <div className="mt-1 grid h-1.5 gap-0.5 sm:gap-1" style={columns} aria-hidden="true">
        {todayIndex >= 0 ? (
          <span
      ...
      <div className="relative mt-1 h-3.5" aria-hidden="true">
        {months.map((m, i) => (
          <span
            key={m.key}
            className={cn(
              'absolute whitespace-nowrap text-caption text-muted-strong',
      ...
      <p id={readoutId} aria-live="polite" className="mt-2 min-h-4 text-footnote text-foreground">
        {readout}
      </p>
