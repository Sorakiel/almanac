...
function Stat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex-1 rounded-2xl border bg-panel px-5 py-[18px]">
      ...
      <p
        className={`mt-1 text-[27px] font-semibold tracking-title tabular-nums ${accent ? 'text-accent' : ''}`}
      >
        {value}
      </p>
    </div>
    ...
    <div className="mx-auto max-w-[900px]">
      <header className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-2 text-[15px] text-muted">Members, activity, and feedback at a glance.</p>
        </div>
      ...
      <p className="label-mono mt-8 mb-3">// signups per week</p>
      ...
      <p className="label-mono mt-8 mb-3">// feedback</p>
