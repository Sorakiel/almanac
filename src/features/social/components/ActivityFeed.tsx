...
import { activitySummary, feedDayLabel, friendName } from '@/features/social/lib/format'
  ...
  return (
    <ul className="flex flex-col gap-2.5">
      {feed.map((item) => {
        ...
        const name = friendName(item.friend, t)
        return (
          <li
            key={item.id}
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm">
                ...
                <span className="text-muted">{activitySummary(item, t)}</span>
              </p>
              <p className="label-mono mt-0.5 text-[10px]">
                {feedDayLabel(item.eventDate, todayKey, t, locale)}
              </p>
            </div>
